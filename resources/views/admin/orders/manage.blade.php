            ...
            <div class="card box">
                ...
                <div class="d-flex justify-content-start gap-3 mt-2">
                    @if ($booking->canceled_at)
                        <div class="py-2"><strong>Order Canceled At:</strong>
                            {{ \Carbon\Carbon::parse($booking->canceled_at)->format('d M Y h:i A') }}</div>
                    @else
                        @can('cancel booking')
                            <button class="btn btn-sm btn_secondary" data-bs-toggle="modal"
                                data-bs-target="#cacelOrderDetailsModal">Cancel Order</button>
                        @endcan
                    @endif
                ...
                <x-modal id="cacelOrderDetailsModal" title="Cancel / Change / Refund Fees" size="modal-lg">
                            ...
                            <div class="card mb-3 shadow-sm">
                                <div class="card-body p-0">
                                    @if ($airline === 'emirates')
                                        @forelse ($booking->bookingItems as $item)
                                            ...
                                            @forelse ($item->penalties as $penalty)
                                                @php
                                                    $cancel = is_array($penalty->cancel_fee)
                                                        ? $penalty->cancel_fee
                                                        : json_decode($penalty->cancel_fee, true);
                                                    $change = is_array($penalty->change_fee)
                                                        ? $penalty->change_fee
                                                        : json_decode($penalty->change_fee, true);
                                                    $refund = is_array($penalty->refund_fee)
                                                        ? $penalty->refund_fee
                                                        : json_decode($penalty->refund_fee, true);
                                                @endphp
                                                <div class="p-3 border-bottom">
                                                    <h6 class="mb-3 text-muted">{{ $penalty->destination }} →
                                                        {{ $penalty->arrival }}</h6>
                                                    @php
                                                        $sections = [
                                                            'Cancel Fees' => $cancel,
                                                                ...
                                                                <table class="table table-sm align-middle mb-3">
                                                                    ...
                                                                    <tbody>
                                                                        @foreach ($data as $when => $fee)
                                                                            <tr>
                                                                                <td>{{ $when }}</td>
                                                                                <td>{{ isset($fee['price']) ? ($fee['price']['amount'] ?? '') . ' ' . ($fee['price']['code'] ?? '') : '' }}
                                                                                </td>
                                                                                <td>{{ $fee['amountApplication'] ?? '' }}
                                                                                </td>
                                                                            </tr>
                                                                        @endforeach
                                                                    </tbody>
                                            ...
                                            @empty
                                                <div class="p-3">
                                                    <p class="text-muted mb-0">No penalties found for this booking item.
                                                    </p>
                                                </div>
                                            @endforelse
                                        @empty
                                    ...
                                    @elseif ($airline === 'flyjinnah')
                                        @php
                                            $xmlBody =
                                                $booking->bookingRequest && isset($booking->bookingRequest->xml_body)
                                                    ? json_decode($booking->bookingRequest->xml_body, true)
                                                    : null;
                                            $airReservation =
                                                $xmlBody['response']['Body']['OTA_AirBookRS']['AirReservation'] ??
                                                ($xmlBody['Body']['OTA_AirBookRS']['AirReservation'] ?? null);
                                            $options =
                                                    ...
                                                    @if (
                                                        ...
                                                        <div class="p-3 border-bottom">
                                                            <h6 class="mb-3 text-muted">
                                                                {{ $segment['DepartureAirport']['@attributes']['LocationCode'] ?? 'N/A' }}
                                                                →
                                                                {{ $segment['ArrivalAirport']['@attributes']['LocationCode'] ?? 'N/A' }}
                                                            </h6>
                                                                ...
                                                                @foreach ($segment['AvailableFlexiOperations']['FlexiOperations'] as $operation)
                                                                    @if (is_array($operation) && !empty($operation['@attributes']))
                                                                        <li class="list-group-item">
                                                                            ...
                                                                            Allowed
                                                                            {{ $operation['@attributes']['NumberOfAllowedOperations'] ?? 'N/A' }}
                                                                            time(s),
                                                                            Cutoff:
                                                                            {{ $operation['@attributes']['FlexiOperationCutoverTimeInMinutes'] ?? 'N/A' }}
                                                                            minutes
                                                                        </li>
                                                                    @endif
                                                                @endforeach
                                                            ...
                                                            <table class="table table-sm align-middle mb-3">
                                                                ...
                                                                <tbody>
                                                                    @foreach ($data as $fee)
                                                                        ...
                                                                        @endphp
                                                                        <tr>
                                                                            <td>{{ $attributes['HoursBeforeDeparture'] ?? 'N/A' }}
                                                                            </td>
                                                                            <td>{{ $attributes['CurrencyCode'] ?? '' }}
                                                                                {{ $attributes['Amount'] ?? '' }}</td>
                                                                        </tr>
                                                                    @endforeach
                                                                </tbody>
                                                            </table>
                    ...
                    <x-slot name="footer">
                        <button type="button" class="btn btn-danger cancelOrderBtn"
                            data-booking-id="{{ $booking->id }}" data-client-id="{{ $booking->client_id }}">Cancel
                            Booking</button>
                    </x-slot>
                </x-modal>
            </div>
        ...
        <div class="col-md-9 right-side">
            ...
            <div class="d-block d-md-flex justify-content-between">
                ...
                <div class="btn-group d-block">
                    @can('booking actions')
                        <button class="btn btn_secondary_outline" data-bs-toggle="modal" data-bs-target="#adminActions">Admin
                            Actions</button>
                    @endcan
                    <button class="btn btn_secondary_outline" data-bs-toggle="modal"
                        data-bs-target="#fareRulesModal">Show Fare Rules</button>
                    ...
                    <button class="btn btn_secondary_outline" data-bs-toggle="modal"
                        data-bs-target="#detailedOverviewModal">Detailed Overview</button>
                    @can('issue tickets')
                        @if ($booking->tickets->isEmpty())
                            <button class="btn btn-outline-dark updatePriceBtn"
                                data-payment-exist="{{ $booking->payments->isEmpty() ? 0 : 1 }}"
                                data-booking-id="{{ $booking->id }}" data-client-id="{{ $booking->client_id }}">Issue
                                Tickets</button>
                        @endif
                    @endcan
                ...
                @can('booking actions')
                    <x-modal id="adminActions" title="Update Booking Fields" size="modal-lg">
                        <form id="update-booking-{{ $booking->id }}" method="POST"
                            action="{{ route('admin.orders.booking.update', $booking) }}">
                            @csrf
                            @method('PUT')
                            <div class="modal-body">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select" name="status">
                                            @foreach ($booking->getStatuses() as $status)
                                                <option value="{{ $status }}"
                                                    {{ $status === $booking->status ? 'selected' : '' }}>{{ $status }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <div class="modal-footer w-100 d-flex justify-content-between">
                                @can('delete bookings')
                                    <button type="button" class="btn btn-danger delete-booking-btn"
                                        data-id="{{ $booking->id }}">
                                        Delete Booking
                                    </button>
                                @endcan
                        ...
                        @can('delete bookings')
                            <form id="delete-booking-{{ $booking->id }}" method="POST"
                                action="{{ route('admin.orders.booking.destroy', $booking) }}" class="d-none">
                                @csrf
                                @method('DELETE')
                            </form>
                        @endcan
                                    ...
                                    if ($airline === 'flyjinnah' && is_array($taxes)) {
                                        $totalTax = array_sum(
                                            array_map(function ($tax) {
                                                return isset($tax['@attributes']['Amount'])
                                                    ? (float) $tax['@attributes']['Amount']
                                                    : 0;
                                            }, $taxes),
                                        );
                ...
                <x-modal id="issueTicketsModal" title="Issue Tickets" size="modal-lg">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn_primary issueTicketBtn"
                            data-booking-id="{{ $booking->id }}" data-client-id="{{ $booking->client_id }}">Approve &
            ...
            <div class="card box">
                <div class="row">
                    ...
                    <div class="col-md-4 py-2"><strong>Order Status:</strong> <span
                            class="badge bg-{{ $booking->status === 'issued' ? 'success' : 'danger' }}">TICKET{{ strtoupper($booking->status) }}</span>
                    </div>
                </div>
                <div class="row mt-2">
            ...
            <div class="card box">
                ...
                <table class="table table-bordered">
                    ...
                    @php
                        $passengers = is_string($booking->passenger_details)
                    ...
                    <tbody>
                        @forelse ($passengers as $passenger)
                            ...
                            @endphp
                            <tr>
                                <td>{{ strtoupper($passenger['title']) }} {{ $passenger['given_name'] ?? '' }}
                                ...
                                @if ($ticket)
                                    ...
                                    <td>
                                        <span
                                            class="badge p-1 bg-{{ $ticket->status === 'success' ? 'success' : 'danger' }}">
                                ...
                                @else
                                    @if ($booking->status === 'issued')
                                        <form action="{{ route('admin.orders.ticket.create') }}" method="POST">
                                            ...
                                            <input type="hidden" name="passenger_reference"
                                                value="{{ $passenger['passenger_reference'] ?? '' }}">
                                            <input type="hidden" name="type"
                                                value="{{ $passenger['type'] ?? '' }}">
                                            <td><input name="ticket_number" type="text"
                                                    class="form-control m-0 p-0 px-2" required></td>
                                            <td colspan="2"><input type="submit" class="form-control m-0 p-0 px-2"
                                                    value="Add ticket"></td>
                                        </form>
                                    @else
                                        <td colspan="3" class="text-center"><span class="text-danger">Not
                                                issued</span></td>
                                    @endif
                                @endif
                            </tr>
                    ...
                    <tbody>
                        @forelse ($booking->payments as $payment)
                            <tr>
                                ...
                                <td>{{ $payment->base_price_code }}
                                    {{ number_format($payment->base_price + $payment->tax, 2) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center">No Payment Found</td>
                            </tr>
                        @endforelse
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#addNoteBtn').on('click', function() {
                ...
                formData.append('booking_type', 'flight');
            ...
            $('#showLogHistoryBtn').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('admin.orders.booking.logs', ':id') }}".replace(':id',
                        bookingId),
                    method: 'GET',
                    data: {
                        booking_type: 'flight'
                    ...
                    success: function(response) {
                        ...
                        if (logs.length === 0) return $('#logHistoryContent').html(
                    ...
                    error: function() {
                        $('#logHistoryContent').html(
                            '<div class="alert alert-danger">Failed to load notes.</div>');
                    }
                });
            });
            // Notes
            ...
            $('.delete-payment-btn').on('click', async function(e) {
                ...
                const ok = await _confirm('Are you sure you want to delete this payment?', true,
                    'warning', 'Yes, delete');
                if (ok) {
            ...
            $('.updatePriceBtn').on('click', async function(e) {
                ...
                if (paymentExist == 0) return _alert('Please add some payments before issue tickets',
                    'warning')
                const ok = await _confirm('Are you sure you want to issue tickets for this flight?',
                    true, 'warning', 'Yes');
                if (ok) {
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            `;
                                ...
                                _alert(res.message || 'Error fetching booking details',
                                    'error');
                            }
                        },
                        error: function(xhr) {
                            _alert(xhr.responseJSON.message ||
            ...
            $('.issueTicketBtn').on('click', async function(e) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status === 'success') {
                            _alert('Booking approved and tickets issued successfully!',
                                'success');
                    ...
                    error: function(xhr) {
                        _alert(xhr.responseJSON.message || 'Error confirming booking',
                            'error');
            ...
            $('.cancelOrderBtn').on('click', async function(e) {
                ...
                if (paymentExist == 0) return _alert('Please add some payments before issue tickets',
                    'warning')
                const ok = await _confirm('Are you sure you want to cancel this booking?', true,
                    'warning', 'Yes');
                if (ok) {
                    $.ajax({
                        ...
                        error: function(xhr) {
                            let errorDetails = xhr.responseJSON.message ||
            ...
            $('.delete-booking-btn').on('click', async function(e) {
                e.preventDefault();
                const id = $(this).data('id');
                const ok = await _confirm('Are you sure you want to delete this booking?', true,
                    'warning', 'Yes, delete');
                if (ok) {
